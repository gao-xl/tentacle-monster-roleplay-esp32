  ...
  <div v-if="isOpen" class="modal-backdrop" @click.self="close">
    <div class="settings-modal">
      <div class="modal-header">
        <div class="title">⚙️ SYSTEM CONFIGURATION // 全局设置中心</div>
      ...
      <div class="modal-body">
        <!-- Tab Navigation -->
        <div class="tabs-nav">
          <button :class="{ active: currentTab === 'ai' }" @click="currentTab = 'ai'">🧠 AI 与大模型预渲染</button>
          <button :class="{ active: currentTab === 'hardware' }" @click="currentTab = 'hardware'">⚡ 硬件与输出安全</button>
          <button :class="{ active: currentTab === 'topology' }" @click="currentTab = 'topology'">🔌 贴片拓扑与性别特调</button>
        </div>
        ...
        <div v-if="currentTab === 'topology'" class="tab-content">
          ...
          <div class="form-group">
            ...
            <select v-model="form.sensitivity_level">
              <option value="DELICATE">极敏感 (Delicate: 细腻酥麻微脉冲)</option>
              <option value="STANDARD">标准冒险家 (Standard)</option>
              <option value="HARDCORE">强忍耐受 (Hardcore: 深度强力压制)</option>
            </select>
          </div>
        </div>
      </div>
...
.tabs-nav button {
  flex: 1; padding: 12px; background: transparent; border: none; color: #8faec9;
...
.form-group input[type="text"]:focus, .form-group select:focus {
  border-color: #00f0ff; box-shadow: 0 0 10px rgba(0, 240, 255, 0.4);
}
